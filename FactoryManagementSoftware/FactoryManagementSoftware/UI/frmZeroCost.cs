...
{
    public partial class frmZeroCost : Form
    {
                    ...
                    foreach (DataRow joinRow in dt_Join.Rows)
                    {
                        if (joinRow.RowState != DataRowState.Deleted)
                        {
                            ...
                            if (childCode == matCode)
                            {
                                ...
                                if (ifCustomerItems(parentCode, dt_JoinforChecking, dt_PMMAItem))
                                {
                    ...
                    foreach (DataRow partRow in dt_Part.Rows)
                    {
                        if (partRow.RowState != DataRowState.Deleted)
                        {
                            ...
                            if (itemMatCode == matCode)
                            {
                                ...
                                if (ifCustomerItems(itemCode,dt_JoinforChecking,dt_PMMAItem))
                                {
        ...
        private void LoadMatUsedData_ByCustomer()
        {
            #region Datatable Data

            //get all pmma item
            string SupplierName = cmbSupplier.Text;
            DataTable dt_CustItem = dalItemCust.custSearch(SupplierName);
            ...
            string customerName = cmbSupplier.Text;

            DataTable dt_PartTrfToCustomerHist = dalTrfHist.rangeItemToCustomerSearch(customerName, from, to);
            #endregion
                    ...
                    foreach (DataRow joinRow in dt_Join.Rows)
                    {
                        if (joinRow.RowState != DataRowState.Deleted)
                        {
                            ...
                            if (childCode == matCode)
                            {
                                ...
                                if (ifCustomerItems(parentCode, dt_JoinforChecking, dt_CustItem))
                                {
                                    string parentName = joinRow["parent_name"].ToString();

                                    //Get delivered out data
                                    dt_DeliveredData.Rows.Clear();
                                    float deliveredQty = DeliveredToCustomerQty(parentCode, dt_JoinforChecking, dt_PartTrfToCustomerHist);
                    ...
                    foreach (DataRow partRow in dt_Part.Rows)
                    {
                        if (partRow.RowState != DataRowState.Deleted)
                        {
                            ...
                            if (itemMatCode == matCode)
                            {
                                ...
                                //check if item belong to customer
                                if (ifCustomerItems(itemCode, dt_JoinforChecking, dt_CustItem))
                                {
                                    string itemName = partRow[dalItem.ItemName].ToString();
                                    ...
                                    //Get delivered out data
                                    dt_DeliveredData.Rows.Clear();
                                    float deliveredQty = DeliveredToCustomerQty(itemCode, dt_JoinforChecking, dt_PartTrfToCustomerHist);
                                    float matUsedInKG = deliveredQty * itemWeight / 1000;
                                    totalMatUsed += (float)Math.Round(matUsedInKG + matUsedInKG * wastage, 2);
            ...
            if (dt_MatUsed.Rows.Count > 0)
            {
                // check if last row has empty MAT CODE
                DataRow lastRow = dt_MatUsed.Rows[dt_MatUsed.Rows.Count - 1];
                string lastMatCode = lastRow[text.Header_MatCode]?.ToString();

                if (string.IsNullOrWhiteSpace(lastMatCode))
                {
                    dt_MatUsed.Rows.RemoveAt(dt_MatUsed.Rows.Count - 1);
                }
        ...
        private DataRow getZeroCostDataRow(string matCode, string month, string year, DataTable dt)
        {
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
        ...
        private void LoadMatStockData()
        {
            ...
            dt_ZeroCost = dalPMMA.SearchByDate(month, year);
            DataTable dt_PMMALastMonth = dalPMMA.SearchByDate(lastMonth, lastYear);

            bool dataLocked = false;

            if(dt_ZeroCost != null && dt_ZeroCost.Rows.Count > 0)
            {
                dataLocked = true;
                foreach (DataRow row in dt_ZeroCost.Rows)
                {
            ...
            else if(cbInOutUpdate.Checked)
            {
                foreach (DataRow row in dt_MatUsed.Rows)
                {
                    ...
                    if (Out != -1 && !string.IsNullOrEmpty(matCode))
                    {
                        float adjustQty = 0;
                        string note = "";
                        DataRow MatData = getZeroCostDataRow(matCode, month, year, dt_ZeroCost);
        ...
        private void LoadMatStockData_ByCustomer()
        {
            #region Setting
            int index = 1;
            dt_MatStock = NewStockTable();
            ...
            DataTable dt_MatInFromSupplierRecord = dalTrfHist.rangeMaterialInFromSupplierSearch(from, to, SupplierName);

            DataTable dt_MatOutToCustomerRecord = dalTrfHist.rangeMaterialOutToCustomerSearch(from, to, CustomerName);
            DataTable dt_ZeroCostLastMonth;

            if (SupplierName == "PMMA")
            {
                dt_ZeroCost = dalPMMA.SearchByDate(month, year);
                dt_ZeroCostLastMonth = dalPMMA.SearchByDate(lastMonth, lastYear);
            }
            else
            {
                dt_ZeroCost = dalPMMA.SearchByDate_BySupplier(month, year, SupplierName);
                dt_ZeroCostLastMonth = dalPMMA.SearchByDate_BySupplier(lastMonth, lastYear, SupplierName);
            }
            ...
            if (dt_ZeroCost != null && dt_ZeroCost.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dt_ZeroCost.Rows)
                {
                    ...
                    if (SupplierName != "PMMA")
                    {
                        isLocked &= row[dalPMMA.Supplier].ToString() == SupplierName;
                    }
            ...
            else if (cbInOutUpdate.Checked)
            {
                foreach (DataRow row in dt_MatUsed.Rows)
                {
                    ...
                    if (Out != -1 && !string.IsNullOrEmpty(matCode))
                    {
                        ...
                        string matName = row[text.Header_MatName].ToString();
                        float openStock = getOpenStockFromLastMonthBal(matCode, dt_ZeroCostLastMonth);
                        ...
                        float outToCustomerQty = getMatOutQty(matCode, dt_MatOutToCustomerRecord);

                        float inQty = getMatInQty(matCode, dt_MatInFromSupplierRecord);

                        float wastage = getMatWastage(matCode, dt_Item);

                        float balance = (float)Math.Round(openStock + inQty - Out - outToCustomerQty + adjustQty, 2);
                        ...
                        row_StockSouce[text.Header_In_KG_Piece] = inQty;
                        row_StockSouce[text.Header_Used_KG_Piece] = Out;
                        row_StockSouce[text.Header_DirectOut_KG_Piece] = outToCustomerQty;
                        row_StockSouce[text.Header_Wastage] = wastage;
                        row_StockSouce[text.Header_Adjust] = adjustQty;
                        row_StockSouce[text.Header_Note] = note;
                        row_StockSouce[text.Header_BalStock] = balance;
                        ...
                        if (Convert.ToInt32(month) == DateTime.Now.Month || Convert.ToInt32(year) > DateTime.Now.Year)
                        {
                            ...
                            uItem.item_updtd_date = DateTime.Now;
                            uItem.item_updtd_by = MainDashboard.USER_ID;
                            bool itemZeroCostQtyUpdateSuccess;

                            if (cmbSupplier.Text == "PMMA")
                            {
                                uItem.item_last_pmma_qty = 0;
                                uItem.item_pmma_qty = balance;

                                itemZeroCostQtyUpdateSuccess = dalItem.UpdatePMMAQty(uItem);
                            }
                            else
                            {
                                uItem.item_last_permabonn_qty = 0;
                                uItem.item_permabonn_qty = balance;

                                itemZeroCostQtyUpdateSuccess = dalItem.UpdatePermabonnQty(uItem);
                            }

                          

                            if (!itemZeroCostQtyUpdateSuccess)
                            {
                                MessageBox.Show("Failed to updated item pmma qty(@item dal)");
                            }
                        }
                        ...
                        uPMMA.pmma_in = inQty;
                        uPMMA.pmma_out = Out;
                        uPMMA.pmma_direct_out = outToCustomerQty;
                        uPMMA.pmma_wastage = wastage;
                        uPMMA.pmma_adjust = adjustQty;
                        uPMMA.pmma_note = note;
                        uPMMA.pmma_bal_stock = balance;
                        ...
                        {
                            uPMMA.pmma_supplier = cmbSupplier.Text;

                            if (!dalPMMA.InsertOrUpdate_byCustomer(uPMMA))
                            {
                                MessageBox.Show("Failed to insert/update data for selected month! (Permabonn)");
                            }
                            else
                            {
                                //insert/update next month open stock
                                ...
                                if (!dalPMMA.InsertOrUpdateNextMonthOpenStock_bySupplier(uPMMA))
                                {
                                    MessageBox.Show("Failed to insert/update open stock for next month! (Permabonn)");
                                }
                            }
                        }
            ...
            else
            {
                foreach (DataRow row in dt_MatUsed.Rows)
                {
                    ...
                    {
                        string matCode = row[text.Header_MatCode].ToString();
                        float adjustQty = 0, outToCustomerQty = 0, inQty = 0;
                        string note = "";
                        DataRow MatData = getZeroCostDataRow(matCode, month, year, dt_ZeroCost);
                        ...
                        if (MatData != null)
                        {
                            ...
                            inQty = float.TryParse(MatData[dalPMMA.PMMAIn].ToString(), out float x) ? x : 0;
                            Out = float.TryParse(MatData[dalPMMA.PMMAOut].ToString(), out x) ? x : 0;
                            outToCustomerQty = float.TryParse(MatData[dalPMMA.DirectOut].ToString(), out x) ? x : 0;
                        }
                        else
                        {
                            outToCustomerQty = getMatOutQty(matCode, dt_MatOutToCustomerRecord);

                            inQty = getMatInQty(matCode, dt_MatInFromSupplierRecord);
                        }
        ...
        private bool ifCustomerItems(string itemCode, DataTable dt_Join, DataTable dt_CustItem)
        {
            bool isCustomerItems = false;
            ...
            foreach(DataRow join in dt_Join.Rows)
            {
                if(itemCode == join["child_code"].ToString())
                {
                    ...
                    isCustomerItems = ifCustomerItems(parentCode, dt_Join, dt_CustItem);
                }
            }

            return isCustomerItems;
        }
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            //LoadMatUsedData();
            LoadMatUsedData_ByCustomer();

            if (!checkMatUsedOnly)
            {
                LoadMatStockData_ByCustomer();
            }
        ...
        private void dgvMatStock_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (editedCellRow != -1 && (adjust != oldAdjustQty || note != oldNote))
            {
                ...
                else
                {
                    ...
                    if (!dalPMMA.InsertOrUpdate_byCustomer(uPMMA))
                    {
                        MessageBox.Show("Failed to update adjusted balance stock! (Permabonn)");
                    }
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //history
                        if (oldAdjustQty != adjust)
                            tool.historyRecord("Permabonn ZERO COST ADJUST", matCode + "(" + monthName + year + ") :" + "ADJUST QTY FROM " + oldAdjustQty + " TO " + adjust, uPMMA.pmma_added_date, uPMMA.pmma_added_by);

                        if (oldNote != note)
                            tool.historyRecord("Permabonn ZERO COST ADJUST", matCode + "(" + monthName + year + ") :" + "NOTE EDITED FROM " + oldNote + " TO " + note, uPMMA.pmma_added_date, uPMMA.pmma_added_by);
                    }
                }
