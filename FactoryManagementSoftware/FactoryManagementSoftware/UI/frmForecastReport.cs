    ...
    public partial class frmForecastReport : Form
    {
        ...
        void BackgroundWorkerProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (e.ProgressPercentage == progressBar1.Maximum)
            {
                ...
                progressBar1.Value = e.ProgressPercentage;           // Set the value
                progressBar1.Value = e.ProgressPercentage - 1;       // Move it backwards
            }
            else
            {
                progressBar1.Value = e.ProgressPercentage + 1;       // Move past
            }
        ...
        historyBLL uHist = new historyBLL();
        historyDAL dalHist = new historyDAL();
        ...
        private void changeBackColor(DataGridViewRow row, int rowIndex, bool type, int dgvType)//if type = true, change previous data back color, else change the new data
        {
            ...
            if (type)
            {
                ...
                if (custType == 1)
                {
                    test = row.Cells[Forecast1ColName].Style.BackColor.ToKnownColor().ToString();
                }
                else
                {
                    test = row.Cells[EstimateColName].Style.BackColor.ToKnownColor().ToString();
                }
                ...
                if (test.Equals("0"))
                {
                    ...
                    {
                        row.Cells[EstimateColName].Style = new DataGridViewCellStyle { BackColor = Color.FromName(colorName) };
                    }
            ...
            else
            {
                ...
                {
                    dgvForecastReport.Rows[rowIndex].Cells[EstimateColName].Style = new DataGridViewCellStyle { BackColor = Color.FromName(colorName) };
                }
        ...
        private bool ifRepeat(string itemCode, int n, string fcast1, string fcast2, string fcast3, string outStock, float shot1, float shot2, int dgvType, float joinQty)
        {
            ...
            foreach (DataGridViewRow row in dgvForecastReport.Rows)
            {
                ...
                if (row.Cells[CodeColName].Value.ToString().Equals(itemCode) && row.Index < n)
                {
                    ...
                    if (cmbCust.Text.Equals(tool.getCustName(1)))//PMMA
                    {
                        type = 1;
                    }
                    else
                    ...
                    {
                        row.Cells[EstimateColName].Value = headFcast1.ToString();
                    }
                    ...
                    if (type == 1)
                    {
                        checkColor = row.Cells[Forecast1ColName].Style.BackColor.ToKnownColor().ToString();
                    }
                    else
                    {
                        checkColor = row.Cells[EstimateColName].Style.BackColor.ToKnownColor().ToString();
                    }
                    ...
                    else
                    {
                        ...
                        {
                            colorName = row.Cells[EstimateColName].Style.BackColor.ToString();
                        }
                    }
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    //MessageBox.Show("total row: "+ dgvForecastReport.RowCount+ "total column: " + dgvForecastReport.ColumnCount);
                    for (int i = 0; i <= dgvForecastReport.RowCount - 1; i++)
                    {
                        //if (dgvForecastReport.Rows[i].Cells[0].Value != null)
                        //    MessageBox.Show(dgvForecastReport.Rows[i].Cells[0].Value.ToString());
        ...
        private void insertAllDataToSheet(string path, string fileName)
        {
            ...
            lblLoading.Visible = true;
            Int32 percentage = 0;
            Int32 savedPercentage = 0;
            bgWorker.ReportProgress(percentage);
            string custName = tool.getCustName(1);

            int totalCust = cmbCust.Items.Count;
            ...
            for (int i = 0; i <= cmbCust.Items.Count - 1; i++)
            {
                ...
                if (gotData)//if datagridview have data
                {
                    ...
                    for (int a = 0; a <= dgvForecastReport.RowCount - 1; a++)
                    {
                        for (int j = 0; j <= dgvForecastReport.ColumnCount - 1; j++)
                        {
                            Range range = (Range)xlWorkSheet.Cells[a + 2, j + 1];
                        ...
                        percentage = (a + 1) * (100 / totalCust) / dgvForecastReport.RowCount;
                        ...
                        else
                        {
                            ...
                            lblLoading.Text = (savedPercentage + percentage).ToString() + "%";
                        }
                    }
