...
{
    public partial class frmSBBMaterialUsedReport : Form
    {
        public frmSBBMaterialUsedReport()
        {
            InitializeComponent();
        ...
        private float SBBDeliveredToCustomerQty(string itemCode, DataTable dt_DOList)
        {
            ...
            foreach (DataRow row_DO in dt_DOList.Rows)
            {
                ...
                if (trfResult == "Passed" && row_DO[dalSPP.ItemCode].ToString() == itemCode)
                {
                    ...
                    DeliveredQty += float.TryParse(row_DO[dalTrfHist.TrfQty].ToString(), out DeliveredQty) ? DeliveredQty : 0;
        ...
        private void LoadMatUsedData_2()
        {
            ...
            DataTable dt_CustItem = dalItemCust.SPPCustSearchWithTypeAndSize(itemCust);

            //DataTable dt_CustItem = dalItemCust.custSearch(custName);
            DataTable dt_DOList = dalSPP.DOWithTrfInfoSelect(from, to);

            //DataTable dt_PartTrfHist = dalTrfHist.rangeItemToCustomerSearch(custName, from, to);
        ...
        private string setFileName()
        {
            if (cmbReportType.Text.Equals(reportType_Delivered))
            {
                return cmbReportType.Text + "_MaterialUsedReport " + "("  + "_" + cmbMonth.Text + "_" + cmbYear.Text + ")_" + DateTime.Now.ToString("ddMMyyyy_HHmmss") + ".xls";

            }
            else if (cmbReportType.Text.Equals(reportType_ReadyStock))
            {
                return cmbReportType.Text + "_MaterialUsedReport " + "(" + ")_" + DateTime.Now.ToString("ddMMyyyy_HHmmss") + ".xls";
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                ...
                xlWorkSheet.PageSetup.CenterHeader = "&\"Calibri,Bold\"&16 ("  + ")" + cmbReportType.Text + "_MATERIAL USED REPORT";
