...
{
    public partial class frmInOutReport_ver2 : Form
    {
        public frmInOutReport_ver2()
        {
            InitializeComponent();
        ...
        public frmInOutReport_ver2(string itemCode, string customer)
        {
            InitializeComponent();
            tool.DoubleBuffered(dgvInOutReport, true);
            tool.loadCustomerToComboBox(cmbItemType);
        ...
        private bool IsCustomer(string trfTo, ComboBox cmbItemType)
        {
            // Validate input parameters
            if (string.IsNullOrWhiteSpace(trfTo) || cmbItemType == null)
            {
                return false;
            }

            // Trim and convert to lowercase for case-insensitive comparison
            string searchText = trfTo.Trim().ToLower();

            // Loop through all items in the combobox
            foreach (var item in cmbItemType.Items)
            {
                // Get the display text of the current item
                string customerName = cmbItemType.GetItemText(item).Trim().ToLower();
        ...
        private void LoadInOutData()
        {
            if (Validation())
            {
                    ...
                    foreach (DataRow trfRow in dt_TrfHist.Rows)
                    {
                        ...
                        if (trfItemCode == itemCode && (listedCustomer || itemType != "SPP" )) //&& (inOutType != inOutType_InOut || inOutType != inOutType_Out)
                        {
                            ...
                            if (cbDeliveredItems.Checked)
                            {
                                ...
                                // Check if "ALL" is selected in the combobox, ignoring case
                                bool isAllItemsSelected = string.Equals(cmbItemType.Text, "ALL", StringComparison.OrdinalIgnoreCase);

                                // Direct comparison if a specific item type is selected, customer check otherwise
                                trfOut = isAllItemsSelected ? IsCustomer(trfTo, cmbItemType) : string.Equals(trfTo, itemType, StringComparison.OrdinalIgnoreCase);
                            }
                ...
                int totalLoadingDuration = frmLoading.GetLastLoadingDuration();

                if(totalLoadingDuration <=0)
                {
                    lblLoadingTime.Text = "Report loaded < 0s" ;

                }
                else
                {
                    lblLoadingTime.Text = "Report loaded in " + frmLoading.GetLastLoadingDuration() + "s";

                }
            }
        }

        private void LoadInOutData_AllCustomer()
        {
            if (Validation())
            {
                #region Pre Setting
                ...
                {
                    dt_TrfHist = dalTrfHist.ItemInOutSearchAllCustomer_NonSBB(start, end);
                }
                else
                {
                    MessageBox.Show("In Out type invaild.");
                ...
                foreach (DataRow itemRow in dt_ItemList.Rows)
                {
                    ...
                    string itemMaterial = itemRow[dalItem.ItemMaterial].ToString();
                    string itemStock = itemRow[dalItem.ItemStock].ToString();
                    string itemCustomer = itemRow["cust_name"].ToString();
                    ...
                    if(itemCustomer != "SPP" && itemCustomer != "OTHER")
                    {
                        foreach (DataRow trfRow in dt_TrfHist.Rows)
                        {
                            ...
                            // skip rows we've marked for deletion
                            if (trfRow.RowState == DataRowState.Deleted)
                                continue;
                            ...
                            string trfFrom = trfRow[dalTrfHist.TrfFrom].ToString();
                            string trfTo = trfRow[dalTrfHist.TrfTo].ToString();
                            string trfID = trfRow[dalTrfHist.TrfID].ToString();
                            ...
                            if (trfItemCode == itemCode)
                            {
                                ...
                                trfIn = CheckIfTrfIn(trfFrom, trfTo);
                                //trfOut = CheckIfTrfOut(trfFrom, trfTo);
                                //trfOut = IsCustomer(trfTo, cmbItemType);
                               
                                if(trfTo == itemCustomer && cbRemoveOutEqual0.Checked ? trfQty == 0 : false)
                                {
                                    trfRow.Delete();
                                    continue;
                                }
                        ...
                        if (itemSearchMatched)
                        {
                            ...
                            if (inOutType == inOutType_In)
                            {
                                row_Out[header_Customer] = itemCustomer;
                            ...
                            else if (inOutType == inOutType_Out)
                            {
                                ...
                                if (totalTrfOutQty == 0 && cbRemoveOutEqual0.Checked)
                                {
                                    continue;
                                }
                            ...
                            {
                                row_In[header_Customer] = itemCustomer;
                                row_Out[header_Customer] = itemCustomer;
        ...
        public DataTable ReorganizeSingleTypeReport(DataTable dt, bool isNonActiveItemsChecked, string reportType)
        {
            ...
            // Column name constants - will match actual column names in the DataTable
            string colIndex = header_Index;          // "#" 
            string colItemCode = header_ItemCode;    // "CODE"
            string colTotal = header_Total;          // "TOTAL"
            string colCustomer = header_Customer;    // "CUSTOMER"

            // Dictionary to keep track of item codes and their total values
            Dictionary<string, double> itemTotals = new Dictionary<string, double>();
            Dictionary<string, DataRow> itemRows = new Dictionary<string, DataRow>();

            // First pass: identify items and their totals
            foreach (DataRow row in dt.Rows)
            {
                // Skip empty rows (separators)
                if (row[colItemCode] == DBNull.Value || string.IsNullOrEmpty(row[colItemCode].ToString()))
                    continue;

                string itemCode = row[colItemCode].ToString();
                string customer = dt.Columns.Contains(colCustomer) ? (row[colCustomer]?.ToString() ?? string.Empty) : string.Empty;

                // Skip "OTHER" customer
                if (dt.Columns.Contains(colCustomer) && string.Equals(customer, "OTHER", StringComparison.OrdinalIgnoreCase))
                    continue;

                // Get total value (might be formatted with "( X BAGS)")
                double totalValue = 0;
                if (dt.Columns.Contains(colTotal))
                {
                    string totalStr = row[colTotal]?.ToString() ?? "0";

                    // Extract numeric part if there's a format like "X ( Y BAGS)"
                    if (totalStr.Contains("("))
                    {
                        totalStr = totalStr.Substring(0, totalStr.IndexOf("(")).Trim();
                    }

                    double.TryParse(totalStr, out totalValue);
                }

                // Store item information
                itemTotals[itemCode] = totalValue;
                itemRows[itemCode] = row;
            }

            // Second pass: build the result table, handling inactive items
            int newIndex = 1;

            foreach (var itemCode in itemRows.Keys)
            {
                // Skip items with zero total if cbRemoveNotActiveItem is checked
                if (isNonActiveItemsChecked && itemTotals[itemCode] == 0)
                {
                    continue; // Skip this inactive item
                }

                DataRow row = resultTable.NewRow();
                CopyRowData(itemRows[itemCode], row);

                // Update index
                if (dt.Columns.Contains(colIndex))
                    row[colIndex] = newIndex;

                // For OUT report with zero total, change customer to "No Out Record"
                if (reportType == inOutType_Out &&
                    itemTotals[itemCode] == 0 &&
                    dt.Columns.Contains(colCustomer))
                {
                    row[colCustomer] = "No Out Record";
                }
        ...
        private void CopyRowData(DataRow source, DataRow destination)
        {
            foreach (DataColumn col in source.Table.Columns)
            {
                if (destination.Table.Columns.Contains(col.ColumnName))
                {
                    try
                    {
                        if (source[col] != DBNull.Value)
                        {
                            destination[col.ColumnName] = source[col];
                        }
                        else
                        {
                            destination[col.ColumnName] = DBNull.Value;
                        }
                    }
                    catch
                    {
                        // If there's a type mismatch, try converting to string
                        if (source[col] != DBNull.Value)
                        {
                            destination[col.ColumnName] = source[col].ToString();
                        }
                        else
                        {
                            destination[col.ColumnName] = DBNull.Value;
        ...
        private void cmbItemType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if(loaded && cbDeliveredItems.Checked)
            {
                ...
                else if (cmbItemType.Text.Equals(tool.getCustName(1)))
                {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if(cmbItemType.Text.ToUpper() == "ALL" && cbDeliveredItems.Checked)
                {
                    LoadInOutData_AllCustomer();
                }
                else
                {
                    LoadInOutData();
        ...
        private void lblLastMonth_Click(object sender, EventArgs e)
        {
            ...
            {
                // Set to last month
                cmbMonthFrom.Text = lastMonth.Month.ToString();
                cmbYearFrom.Text = lastMonth.Year.ToString();
                getStartandEndDate();
            }
            else if (dateType == dateType_Monthly)
            {
