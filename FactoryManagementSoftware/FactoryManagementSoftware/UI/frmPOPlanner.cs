...
{
    public partial class frmPOPlanner : Form
    {
        public frmPOPlanner()
        {
            InitializeComponent();
        ...
        readonly string text_Brand = "Brand";
        ...
        private void LoadBrandCMB(ComboBox cmb)
        {
            DataTable dt = new DataTable();
            dt.Columns.Add(text_Brand);

            dt.Rows.Add(text.SBB_BrandName);
            dt.Rows.Add(text.OEM_BrandName);

            cmb.DataSource = dt;
            cmb.DisplayMember = text_Brand;
        ...
        {
            ShowOrHideFilter(btnFilter.Text == text_ShowFilter);
        }

        private void ShowOrHideFilter(bool show)
        {
            ...
            if (show)
            {
                ...
                tlpPoList.RowStyles[1] = new RowStyle(SizeType.Absolute, 200f);
            }
            else
            {
                btnFilter.Text = text_ShowFilter;

                tlpPoList.RowStyles[1] = new RowStyle(SizeType.Absolute, 1f);
            }
        }
        ...
        private void dgvList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if((fillStatus == text_SelectCol || fillStatus == text_FillIn) && dgvList.SelectedColumns.Count > 0)
            {
                ...
                {
                    FILL_STATUS = text_FillIn;
                }
                else
                {
                    FILL_STATUS = text_SelectCol;
                }
            ...
            if ((DOStatus == text_SelectCol || DOStatus == text_OpenDO) && dgvList.SelectedColumns.Count > 0)
            {
                ...
                if (IfColSelected(dgv))
                {
                    DO_STATUS = text_OpenDO;
                }
                else
                {
                    DO_STATUS = text_SelectCol;
                }
        ...
        private void LoadSBBCustomerList(ComboBox cmb)
        {
            ...
            dt.DefaultView.Sort = dalSPP.ShortName + " ASC";
            dt = dt.DefaultView.ToTable();


            DataRow dr;
            dr = dt.NewRow();
            dr[dalSPP.ShortName] = "ALL";
