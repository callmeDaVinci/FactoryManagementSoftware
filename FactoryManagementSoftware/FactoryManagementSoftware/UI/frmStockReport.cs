...
namespace FactoryManagementSoftware.UI
{
    public partial class frmStockReport : Form
    {
        ...
        private void createDatagridview()
        {
            ...
            tool.AddTextBoxColumns(dgv, "TOTAL", totalStockColumnName, DisplayedCells);
            dgv.Columns[totalStockColumnName].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            ...
            else
            {
                ...
                dgv.Columns[totalStockColumnName].ReadOnly = true;
        ...
        private bool loadMaterialStockData()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow item in dt.Rows)
                {
                    ...
                    {
                        int n = dgvNewStock.Rows.Add();
                        dataInsertToStockDGV(dgvNewStock, item, n, index_Public.ToString());
                        index_Public++;
                    }
                }
            }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        private void dgvStockReport_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            int columnIndex = dgvNewStock.CurrentCell.ColumnIndex;
            string columnName = dgvNewStock.Columns[columnIndex].Name;
        ...
        {
            var itemCode = dgvNewStock[codeColumnName, e.RowIndex].Value;
            var oldValue = dgvNewStock[e.ColumnIndex, e.RowIndex].Value;
            var newValue = e.FormattedValue;
            var total = dgvNewStock[totalStockColumnName, e.RowIndex].Value;
            var unit = dgvNewStock[unitColumnName, e.RowIndex].Value;
            string facName = dgvNewStock.Columns[e.ColumnIndex].HeaderText;
            ...
            if (total != null)
            {
                if(float.TryParse(dgvNewStock[totalStockColumnName, e.RowIndex].Value.ToString(), out float i))
                {
            ...
            if (!oldValue.ToString().Equals(newValue.ToString()) && !string.IsNullOrEmpty(newValue.ToString()))
            {
                ...
                if(outSuccess && inSuccess)
                {
                    ...
                    dgvNewStock[totalStockColumnName, e.RowIndex].Value = newTotal;

                    if(newTotal < 0)
                    {
                        dgvNewStock[totalStockColumnName, e.RowIndex].Style.ForeColor = Color.Red;
                    }
                    else
                    {
                        dgvNewStock[totalStockColumnName, e.RowIndex].Style.ForeColor = Color.Black;
