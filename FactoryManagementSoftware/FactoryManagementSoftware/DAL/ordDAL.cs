...
namespace FactoryManagementSoftware.DAL
{
    class ordDAL
    {
        ...
        public DataTable RequestingOrPendingOrderSelect()
        {
            //static methodd to connect database
            SqlConnection conn = new SqlConnection(myconnstrng);
            //to hold the data from database
            ...
            {
                //sql query to get data from database
                String sql = "SELECT * FROM tbl_ord INNER JOIN tbl_item ON tbl_ord.ord_item_code = tbl_item.item_code WHERE (ord_status=@REQUESTING OR ord_status=@PENDING)";
                //for executing command
                SqlCommand cmd = new SqlCommand(sql, conn);

                cmd.Parameters.AddWithValue("@REQUESTING", "REQUESTING");
