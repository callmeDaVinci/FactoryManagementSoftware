...
{
    public partial class frmOUGNewPO : Form
    {
        public frmOUGNewPO()
        {
            InitializeComponent();
        ...
        public frmOUGNewPO(string poCode)//edit po
        {
            InitializeComponent();
            customerTblCode = "";
            dt_CustomerList = dalData.CustomerWithoutRemovedDataSelect();
        ...
        private void LoadSizeData(DataTable dt_Size, string itemType)
        {
            ...
            foreach (DataRow row in dt_Size.Rows)
            {
                ...
                if (!isRemoved)
                {
                    ...
                    if ((itemType.Contains(text.SBB_TYPE_EQUAL) || itemType.Contains(text.SBB_TYPE_ENDCAP) || itemType.Contains(text.SBB_TYPE_REDUCING)) && numerator < 20)
                    {
                        sizeFilter = false;
                    }
            ...
            if (dt_CMB.Rows.Count > 0)
            {
                ...
                if(!itemType.Contains(text.SBB_TYPE_EQUAL) && !itemType.Contains(text.SBB_TYPE_ENDCAP))
                {
